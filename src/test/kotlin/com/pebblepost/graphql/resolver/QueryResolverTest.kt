...
class QueryResolverTest {
    ...
    @Autowired
    lateinit var queryResolver: PostResolver

    @Test
    fun testAddPost() {
        var  myPost = Post("eric chang", "graphql", "technology")
        postRepository.save(myPost)

        postRepository.findAll().forEach { println(it) }

        queryResolver.posts().forEach {println("query resolve return: ${it.author}")}
    }
}